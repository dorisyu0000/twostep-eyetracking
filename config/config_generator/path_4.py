...
combinations = list(product(number_range, repeat=8))  # Adjusted for the number of contributing nodes

def calculate_difficulty(combination):
    # Extract the ratings for each path
    root_1, root_2, root_3, chidl_11,child_21, child_22, child_31, child_32 = combination

    # Calculate the sum of the ratings for each path influenced by these nodes
    path1 = root_1 + chidl_11
    path2 = root_2 + child_21
    path3 = root_2 + child_22
    path4 = root_3 + child_31
    path5 = root_3 + child_32
...
with open(os.path.join(dest, "trials_4.json"), "w") as file:
